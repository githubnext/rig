...
The budget includes the initial attempt and all retries. `repair` is an addon value, not a factory: use `addons: repair`, never `repair(...)`, and configure `maxTurns` on the agent spec.
